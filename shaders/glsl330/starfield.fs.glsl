...
float getDecide(vec2 uv, float centerX, float yOffset, float speedFactor, float lengthFactor, float widthFactor) {
    ...
    float startY = mod(iTime / scrollFactor, 1.0 + height + yOffset) - height;
...
vec4 mixStarsLayer(vec2 uv, vec4 c, vec4 bg, float speedFactor, float lengthFactor, float widthFactor, int starsCount, vec4 starColor, float offsetX) {
    float starsGap = 1.0 / starsCount;
...
void main() {
    ...
    **/
    ...
    if (iDrawingBg == 1) {
        c = mixStarsLayer(uv, c, bg, speedFactor, 1.0, 1.0, starsCount, currentStarColor, -.05);
    }
    ...
    **/
    ...
    c = mixStarsLayer(uv, c, bg, speedFactor, lengthFactor, widthFactor, starsCount, currentStarColor, offsetX);
